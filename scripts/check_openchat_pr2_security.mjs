...
if (new Date().toISOString().slice(0, 10) > policy.expiresOn) {
  failures.push(`Security baseline expired on ${policy.expiresOn}; re-audit ${policy.trackingIssue}`);
}

function changedNpmManifests() {
  const diff = git(["diff", "--name-only", policy.baseCommit, "--", "frontend"]);
  if (diff.status !== 0) throw new Error(`npm delta check failed: ${diff.stderr}`);
  const untracked = git(["ls-files", "--others", "--exclude-standard", "--", "frontend"]);
  if (untracked.status !== 0) throw new Error(`npm untracked check failed: ${untracked.stderr}`);
...
if (modes.has("rust")) {
  ...
  if (introduced.length) failures.push(`PR2 introduced external Cargo package/version tuples:\n  ${introduced.join("\n  ")}`);
  console.log(`RustSec: exact ${policy.rust.vulnerabilities.length} vulnerabilities / ${policy.rust.warnings.length} warnings; external lock delta ${introduced.length}`);
}

if (modes.has("licenses")) {
  const metadata = json(run(process.platform === "win32" ? "cargo.exe" : "cargo", ["metadata", "--format-version", "1"]), "cargo metadata");
