...
test("context binds the release to exact checkout SHA and supported GitHub origin", () => {
    ...
    for (const changes of [
        ...
        assert.throws(() => releaseCheckContext({ ...env, ...changes }, context.sha));
...
test("latest attempt must pass; an older green run cannot mask a queued/failed rerun", () => {
    assert.equal(latestSuccessfulRun(page(run()), "frontend.yaml", context).id, 42);
    assert.throws(() =>
        latestSuccessfulRun(
            page(
                run(),
...
for (const changes of [
    ...
    test(`rejects untrusted/incomplete run evidence ${JSON.stringify(changes)}`, () => {
        assert.throws(() =>
            latestSuccessfulRun(page(run("frontend.yaml", changes)), "frontend.yaml", context),
        );
    });
}
...
test("success requires every job, not an entirely skipped workflow", () => {
    ...
    assert.throws(() => assertSuccessfulJobs({ total_count: 0, jobs: [] }, run(), context));
});

test("fetches exact workflow paths and attempt jobs using bounded read-only requests", async () => {
    const calls = [];
    const results = await verifyHostedChecks(context, "test-token", async (url, options) => {
        calls.push(url);
        assert.equal(options.redirect, "error");
        assert.ok(options.signal);
        assert.equal(options.headers.Authorization, "Bearer test-token");
        if (url.includes("/attempts/1/jobs?")) return { ok: true, json: async () => jobPage() };
...
test("API failures or missing authentication never become a pass", async () => {
    ...
    await assert.rejects(
        verifyHostedChecks(context, "test-token", async () => ({ ok: false, status: 403 })),
        /HTTP 403/u,
    );
    await assert.rejects(
...
test("Android build and upload are preceded by prerequisite checks", () => {
    ...
    const recheck = workflow.indexOf("- name: Recheck exact-source release prerequisites");
    assert.ok(recheck > workflow.indexOf("- name: Build Android APK (Store)"));
    assert.ok(recheck < workflow.indexOf("- name: Upload APKs"));
});
