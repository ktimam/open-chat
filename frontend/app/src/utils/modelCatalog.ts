...
export const defaultModelCatalog: ModelCatalog = {
    ...
    models: [
        ...
        //
        // A vision model sits here because it measured BEST AT TEXT TOO, not merely because it can
        // read pictures: against the app's real extraction prompt it was the only model of five to
        // apply the date-range rule ("3-8 august" → the 3rd) and the only one to return an amount
        // from a photo as a JSON number rather than a string. It costs ~2x Gemma 3 1B's download;
        // the text-only alternatives below stay one click away. Numbers: fork-notes/09-browser-vision.md.
        {
        ...
        //
        // Every vision entry here was measured in a real browser against the app's real extraction
        // prompt, not chosen from benchmarks. SmolVLM 256M was measured too and REJECTED: it emitted
        // nested, structurally invalid JSON on the image AND on a text-only control, so it could never
        // produce a card. Numbers and method: fork-notes/09-browser-vision.md.
        {
            ...
            description:
                ...
                "as an amount of 3.00. Check every figure before you confirm.",
            modalities: ["text", "image"],
            runtime: "llama-cpp",
            files: [
                {
...
// though its original justification did not. Method and numbers: fork-notes/09-browser-vision.md.
//
