...
export function validAppProcessorCandidates(value: unknown): value is Record<string, unknown>[] {
    return Array.isArray(value) && value.length > 0 && value.length <= MAX_AI_ACTION_CANDIDATES &&
        value.every((item) => record(item) && safeJson(item)) && bytes(value) <= APP_PROCESSOR_MAX_BYTES;
}

export function parseAppProcessorResult(value: unknown, expected: Binding): AppProcessorResult | undefined {
    if (!record(value) || value.type !== PREFIX + "result" || value.version !== VERSION ||
        value.frameNonce !== expected.frameNonce || value.requestNonce !== expected.requestNonce ||
        Object.keys(value).some((key) => !["type", "version", "frameNonce", "requestNonce", "kind", "candidates"].includes(key)) ||
        bytes(value) > APP_PROCESSOR_MAX_BYTES) return undefined;
    if (value.kind === "candidates" && validAppProcessorCandidates(value.candidates)) {
        return { kind: "candidates", candidates: value.candidates };
    }
    if (!Object.hasOwn(value, "candidates") && (value.kind === "none" || value.kind === "ambiguous")) {
...
export function abortAppLocalProcessors(): void { for (const cancel of [...active]) cancel(); }
...
export async function processWithApp(
    ...
    const unavailable = (): AppProcessorResult => ({ kind: "error", error: "The app's local processor is unavailable. Refresh and retry." });
    const viewer = currentUserIdStore.value;
    const current = () => currentUserIdStore.value === viewer && stillCurrent();
    if (!current() || !supportsCredentiallessIframe() || active.size >= MAX_ACTIVE ||
        (input.text !== undefined && new TextEncoder().encode(input.text).byteLength > MAX_SOURCE_BYTES) ||
        ...
        !safeJson(input) || bytes(input) > APP_PROCESSOR_MAX_BYTES) return unavailable();
    let url: URL;
    try { url = new URL(registeredCardUrl); } catch { return unavailable(); }
    if (!["https:", "http:"].includes(url.protocol) || url.username || url.password ||
        (url.protocol === "http:" && !["127.0.0.1", "localhost"].includes(url.hostname))) return unavailable();
    ...
    return new Promise((resolve) => {
        ...
        const bootstrap = () => {
            ...
            if (!sent) frame.contentWindow?.postMessage({ type: PREFIX + "bootstrap", version: VERSION, ...binding }, "*");
        };
        const receive = (event: MessageEvent) => {
            if (settled || event.source !== frame.contentWindow || event.origin !== "null") return;
            if (!current()) return cancel();
            const data: unknown = event.data;
            if (!record(data) || data.frameNonce !== binding.frameNonce || data.requestNonce !== binding.requestNonce || data.version !== VERSION) return;
            if (!sent && data.type === PREFIX + "ready" && Object.keys(data).length === 4) {
                sent = true;
                frame.contentWindow?.postMessage({ type: PREFIX + "request", version: VERSION, ...binding, actionId, input }, "*");
