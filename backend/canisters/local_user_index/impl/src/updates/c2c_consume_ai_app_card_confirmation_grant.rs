...
use local_user_index_canister::c2c_consume_ai_app_card_confirmation_grant::{Response::*, *};
...
async fn c2c_consume_ai_app_card_confirmation_grant(args: Args) -> Response {
    ...
    let result = user_index_canister_c2c_client::c2c_consume_ai_app_card_confirmation_grant(
        read_state(|state| state.data.user_index_canister_id),
        &user_index_canister::c2c_consume_ai_app_card_confirmation_grant::Args {
            context,
    ...
    if current_registration != caller_registration {
        return InvalidRequest("local child registration changed while confirmation grant was consumed".to_string());
    }
    if let Err(error) = validate_authoritative_child_context(
        expected_user_id,
        expected_chat,
    ...
    match result {
        Ok(user_index_canister::c2c_consume_ai_app_card_confirmation_grant::Response::Success) => Success,
        Ok(user_index_canister::c2c_consume_ai_app_card_confirmation_grant::Response::NotFound) => NotFound,
        Ok(user_index_canister::c2c_consume_ai_app_card_confirmation_grant::Response::Expired) => Expired,
        Ok(user_index_canister::c2c_consume_ai_app_card_confirmation_grant::Response::AppUnavailable) => AppUnavailable,
        Ok(user_index_canister::c2c_consume_ai_app_card_confirmation_grant::Response::InvalidRequest(error)) => {
            InvalidRequest(error)
        }
        Err(_) => Error("confirmation grant service unavailable".to_string()),
    }
}
