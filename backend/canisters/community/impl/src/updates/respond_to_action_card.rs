...
fn prepare(args: &Args, state: &mut RuntimeState) -> OCResult<Prepared> {
    ...
    if matches!(args.response, ActionCardResponse::Confirm) {
        ...
        // Enforce channel-local app policy before reservation. A failed source lookup may be a
        // legacy, routing-less card, so reserve/ordinary response remains authoritative for it.
        if let Ok(source) =
            channel
                .chat
                .ai_app_card_confirmation_source(user_id, args.thread_root_message_index, args.message_id, now)
            && !channel.enabled_ai_apps.contains(&source.app_id)
        {
...
fn current_confirmation_grant_hash(
    ...
    let channel = state.data.channels.get_or_err(&channel_id)?;
    channel.chat.events.action_card_confirmation_grant_hash_for_lease(
        deposit.context.thread_root_message_index,
        deposit.context.message_id,
        EventIndex::default(),
        user_id,
...
fn persist_consumed_grant(
    ...
    channel.chat.events.mark_action_card_confirmation_grant_consumed(
        deposit.context.thread_root_message_index,
        deposit.context.message_id,
        EventIndex::default(),
        user_id,
...
// None means the exact one-use bearer was already consumed for this persisted lease and must not
// be sent to UserIndex again. A different bearer can never inherit that marker.
...
mod tests {
    ...
    fn boxed_prepared_deposit_preserves_the_owned_instruction() {
        ...
        let context = super::ActionDepositContext {
            chat: types::Chat::Channel(canister_id.into(), 14u32.into()),
