...
use community_canister::create_ai_app_private_match_capability::{Response::*, *};
...
impl Prepared {
    fn binding(&self) -> group_index_canister::ai_app_card_authority::AiAppCardAuthorityBindingV1 {
        let Chat::Channel(community_id, channel_id) = self.args.chat else {
            unreachable!("community private match prepared a non-channel chat")
        };
        group_index_canister::ai_app_card_authority::AiAppCardAuthorityBindingV1 {
            ...
            context: types::AiAppCardContext {
                ...
                chat: self.args.chat,
                chat_key: format!("channel:{community_id}:{channel_id}"),
...
fn prepare(args: &Args, state: &RuntimeState) -> Result<Prepared, oc_error_codes::OCError> {
    ...
    let source =
        channel
            .chat
            .ai_app_private_match_source(user_id, args.thread_root_message_index, args.message_id, state.env.now())?;
    Ok(Prepared {
...
fn revalidate(prepared: &Prepared, state: &RuntimeState) -> Result<(), oc_error_codes::OCError> {
    ...
    if !channel.enabled_ai_apps.contains(&prepared.args.app_id) {
        return Err(oc_error_codes::OCErrorCode::InvalidRequest.with_message("the app is no longer enabled"));
    }
    let source = channel.chat.ai_app_private_match_source(
