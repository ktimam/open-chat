...
impl AiAppScopedIdentityKey {
    pub fn ensure_initialized(&mut self, rng: &mut (impl CryptoRng + RngCore)) -> Result<bool, String> {
        if self.0.is_empty() {
    ...
    pub fn consumer_queue_selector(
        ...
        preimage.extend_from_slice(public_key.to_encoded_point(false).as_bytes());
        self.mac(&preimage)
    }
...
mod tests {
    ...
    fn queue_selector_requires_the_secret_even_with_public_pem_and_route_coordinates() {
        ...
        public_attempt.extend_from_slice(parsed.to_encoded_point(false).as_bytes());
