...
fn attempt_reseed() {
    ...
    match admission {
        ...
        Pr2EntropyReseedAdmission::Started(ticket) => {
            ...
            // Checkpoint uses ic-cdk 0.18, whose established spawn is required here.
...
fn schedule_watchdog(ticket: Pr2EntropyReseedTicket, delay_ms: u64) {
    let _ = ic_cdk_timers::set_timer(Duration::from_millis(delay_ms), move || check_watchdog(ticket));
}
