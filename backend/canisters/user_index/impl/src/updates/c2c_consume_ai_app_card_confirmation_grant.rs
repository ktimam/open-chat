...
async fn c2c_consume_ai_app_card_confirmation_grant(args: Args) -> Response {
    ...
    mutate_state(|state| {
        ...
        let Ok(chat_key) = canonical_card_chat_key(args.context.user_id, args.context.chat) else {
            return InvalidRequest("invalid canonical chat identity".to_string());
        };
        ...
        if matches!(grant.context.chat, types::Chat::Direct(_)) && !app.manifest.per_user_keys {
            return AppUnavailable;
        }
