...
fn accept_if_valid(state: &RuntimeState) {
    ...
    let is_valid =
        action_signing_key_lifecycle_policy(&method_name, state.is_caller_governance_principal()).unwrap_or_else(|| {
...
fn action_signing_key_lifecycle_policy(method_name: &str, governance_principal: bool) -> Option<bool> {
    matches!(method_name, "stage_action_signing_key" | "activate_action_signing_key").then_some(governance_principal)
}
...
mod tests {
    ...
    fn action_signing_key_lifecycle_ingress_is_governance_only() {
        for method in ["stage_action_signing_key", "activate_action_signing_key"] {
            assert_eq!(action_signing_key_lifecycle_policy(method, true), Some(true));
            assert_eq!(action_signing_key_lifecycle_policy(method, false), Some(false));
        }
        assert_eq!(action_signing_key_lifecycle_policy("publish_ai_app", true), None);
    }
    ...
    fn action_signing_key_lifecycle_msgpack_aliases_use_the_same_policy() {
        for method in ["stage_action_signing_key_msgpack", "activate_action_signing_key_msgpack"] {
            ...
            assert_eq!(action_signing_key_lifecycle_policy(normalized, true), Some(true));
            assert_eq!(action_signing_key_lifecycle_policy(normalized, false), Some(false));
        }
    }
}
