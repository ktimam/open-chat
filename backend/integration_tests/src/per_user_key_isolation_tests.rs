...
// A per-user app routes one confirmation only to the actual confirmer's opaque selector. Another
// linked member receives nothing and cannot decrypt the confirmer's envelope.
#[test]
fn confirmer_delivery_is_encrypted_and_isolated_from_other_members() {
    ...
    assert_eq!(actions_a.len(), 1, "the actual confirmer must receive one encrypted delivery");
    assert_eq!(actions_b.len(), 0, "a linked nonconfirmer must receive no delivery");

    let plaintext_a = decrypt(&actions_a[0], &recipient_a.sk_pem).expect("A must decrypt its confirmer envelope");
    ...
    assert!(
        ...
        "a nonconfirmer's key must not decrypt the confirmer's envelope"
    );
}
