...
use hkdf::Hkdf;
use p256::PublicKey;
use p256::elliptic_curve::{Generate, rand_core::CryptoRng, sec1::ToSec1Point};
...
pub fn action_signing_key_id(public_key_pem: &str) -> Result<[u8; ACTION_SIGNING_KEY_ID_BYTES], String> {
    ...
    digest.update(public_key.to_sec1_point(false).as_bytes());
...
pub fn key_fingerprint(public_key_pem: &str) -> Result<[u8; 32], String> {
    ...
    Ok(Sha256::digest(pk.to_sec1_point(false).as_bytes()).into())
}

/// Encrypt `plaintext` to `recipient_public_key_pem` (a P-256 SubjectPublicKeyInfo PEM).
pub fn encrypt(plaintext: &[u8], recipient_public_key_pem: &str, rng: &mut impl CryptoRng) -> Result<EciesEnvelope, String> {
    let recipient_pk = PublicKey::from_public_key_pem(recipient_public_key_pem).map_err(|e| e.to_string())?;

    let ephemeral_sk = p256::SecretKey::generate_from_rng(rng);
    ...
    Ok(EciesEnvelope {
        ephemeral_public_key: ephemeral_pk.to_sec1_point(false).as_bytes().to_vec(),
        ciphertext,
    })
}
...
mod tests {
    ...
    fn encrypt_then_decrypt_round_trips() {
        ...
        let recipient = p256::SecretKey::generate_from_rng(&mut rng);
    ...
    fn print_interop_vector() {
        ...
        let oc = p256::SecretKey::generate_from_rng(&mut rng);
    ...
    fn wrong_recipient_cannot_decrypt() {
        ...
        let recipient = p256::SecretKey::generate_from_rng(&mut rng);
        let attacker = p256::SecretKey::generate_from_rng(&mut rng);
        let pk_pem = recipient.public_key().to_public_key_pem(Default::default()).unwrap();
